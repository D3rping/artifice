...
public class ArtificeResourcePackImpl implements ArtificeResourcePack {
    ...
    public Collection<Identifier> findResources(ResourceType type, String rootFolder, String idk, int max, Predicate<String> filter) {
    ...
    public ArtificeResourcePackProfile getAssetsProfile(ResourcePackProfile.Factory<?> factory) {
        return new ArtificeResourcePackProfile(this.optional, this.visible, ResourcePackProfile.of(
    ...
    public ResourcePackProfile getDataProfile(ResourcePackProfile.Factory<?> factory) {
    ...
    public void dumpResources(String folderPath) throws IOException, IllegalArgumentException {
        LogManager.getLogger().info("[Artifice] Dumping "+getName()+" "+type.name()+" to "+folderPath+", this may take a while.");
        ...
        resources.forEach((id, resource) -> {
            String path = String.format("./%s/%s/%s/%s", folderPath, this.type.name(), id.getNamespace(), id.getPath());
            writeResourceFile(new File(path), resource);
        });

        LogManager.getLogger().info("[Artifice] Finished dumping "+getName()+" "+type.name()+".");
    }
